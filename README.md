...
2. **添加正则表达式规则**
   ```
   /ga add /\d{11}/           # 匹配11位数字（手机号）
   /ga add /[1-9][0-9]{4,}/   # 匹配QQ号
   ```
...
| 指令 | 说明 | 权限 |
|------|------|------|
| `/ga add [关键词|正则]` | 添加验证规则 | 管理员 |
| `/ga remove [索引]` | 删除指定规则 | 管理员 |
| `/ga list` | 查看当前群规则 | 所有用户 |
| `/ga clear` | 清空所有规则 | 管理员 |
| `/ga test [文本]` | 测试文本匹配 | 所有用户 |
| `/ga whitelist add [ID]` | 添加用户到白名单 | 管理员 |
| `/ga whitelist remove [ID]` | 从白名单移除用户 | 管理员 |
| `/ga whitelist list` | 查看白名单 | 所有用户 |
| `/ga blacklist add [ID]` | 添加用户到黑名单 | 管理员 |
| `/ga blacklist remove [ID]` | 从黑名单移除用户 | 管理员 |
| `/ga blacklist list` | 查看黑名单 | 所有用户 |
| `/ga help` | 显示帮助信息 | 所有用户 |
...
### 场景5: 白名单管理
```
/ga whitelist add 123456789
/ga whitelist add 987654321
```
添加特定用户到白名单，这些用户将直接通过验证

### 场景6: 黑名单管理
```
/ga blacklist add 111222333
/ga blacklist add 444555666
```
...
- **作者**: Kush-ShuL
- **项目地址**: https://github.com/Kush-ShuL/GroupAdminer
